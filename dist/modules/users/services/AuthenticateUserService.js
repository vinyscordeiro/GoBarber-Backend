...
let AuthenticateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const passMatch = await this.hashProvider.compareHash(password, user.password); // await compare(password, user.password);
    ...
    const token = (0, _jsonwebtoken.sign)({}, secret, {
      subject: user.id,
      expiresIn // Está utilizando a short syntax, ou seja, expiresIn : expiresIn (Variavel).
